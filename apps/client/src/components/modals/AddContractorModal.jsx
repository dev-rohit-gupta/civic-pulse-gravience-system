...
const AddContractorModal = ({ onClose }) => {
  const { handleAddContractor } = useApp();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const success = handleAddContractor(contractorForm);
    if (success) {
      onClose();
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg p-8 max-w-md w-full">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Add New Contractor</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setContractorForm({ ...contractorForm, department: e.target.value })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setContractorForm({ ...contractorForm, rating: e.target.value })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              placeholder="0-5"
            />
          </div>
